...
public class EmployeeServiceImpl implements EmployeeService {
	...
	public void addPhoneToEmployee(int EmployeeId, Phone phone) {	
        ...
        if (currentEmployee==null) {
            logger.info ("addPhoneToEmployee : Employee with id " + EmployeeId + " not found");
            return;
        }

        List<Phone> phones = currentEmployee.getPhoneNumbers();
        phones.add(phone);
        
        List<Employee> employees = phone.getEmployees();
        employees.add(currentEmployee);
        
        phone.setEmployees(employees);
        
        currentEmployee.setPhoneNumbers(phones);        
        try {
            dao.updateEmployee(currentEmployee);
            registrarMovimiento("MODIFICACION", currentEmployee);
        }
          catch (Exception e){
        	  logger.error("addPhoneToEmployee, employee id=" + EmployeeId + 
        			  ",\n e.toString(): " + e.toString() + 
        			  ",\n e.getMessage(): " + e.getMessage() + 
			          ",\n e.getCause().getMessage(): " + e.getCause().getMessage());
          }
	...
	public void deletePhoneToEmployee (Phone phone, int EmployeeId){
        Employee currentEmployee = findById(EmployeeId);
        
        if (currentEmployee==null) {
            logger.info ("deletePhoneToEmployee : Employee with id " + EmployeeId + " not found");
            return;
        }		
        List<Phone> lphone = currentEmployee.getPhoneNumbers();
        
        Iterator<Phone> it = lphone.iterator();
        Boolean found = false;
        while (it.hasNext() && !found) {
            Phone phoneItem = it.next();
            if (phoneItem.getId() == phone.getId()) {
                it.remove();
                found = true;
            }
        }
        if (found == true){
            try {
                ...
                registrarMovimiento("MODIFICACION", currentEmployee);
            	  ...
            	  logger.error("deletePhoneToEmployee, employee id=" + EmployeeId + 
            			  ",\n e.toString(): " + e.toString() + 
            			  ",\n e.getMessage(): " + e.getMessage() + 
    			          ",\n e.getCause().getMessage(): " + e.getCause().getMessage());
              }        	
