...
public class PhoneDaoImplTest extends EntityDaoImplTest{
	...
	@Rollback(false)
	void updatePhone(){
		Phone p = getPhoneId (new Long(1));
		System.out.println("phone(1) :" +  p.toString());
		p.setContactnumber("1100110099");
		phoneDao.mergePhone(p);
		Phone p2 = getPhoneId (new Long(1));
		Assert.assertEquals(p2.getContactnumber(),"1100110099");
		
	}
	
	
	public Phone getPhoneId(Long id ){
		Phone p = phoneDao.findById(id);
		return p;
